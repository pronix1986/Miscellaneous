...
define a_p_id=&6
prompt $> Enter the Id of current approver(If you enter '0' then all approvers of current Action plan will be deleted and changed at new assignee)
define curr_user_id=&7
prompt $> Enter the Id of new assignee 
define new_user_id=&8

spool reassign_approver.txt;
...
BEGIN
	...
	DBMS_OUTPUT.PUT_LINE('curr_user_id       curr_user_name       Enabled        Role_ACTIONPLAN_APPROVER        Primary_Business_Unit');
	DBMS_OUTPUT.PUT_LINE('------------------------------------------------------------------------------------------------------------');
	...
	open cur for select inf_obj_id, vu.username, decode(vu.enabled,  0, 'disabled', 1, 'enabled', 'unknown'), 
		...
		(select u.role_id from v_user_role_env u where u.user_id = vu.inf_obj_id) and vr.name = 'ACTIONPLAN_APPROVER')), 
		'true', 'false'),
		(select vd.value_id from v_dimension vd where vd.value_type = 'PrimaryBusinessUnit' and vd.dimension_node_id in
		(select dimension_node_id from v_user_role_env where role_id = 211 and user_id = vu.inf_obj_id))
		from v_user vu
	...
	CLOSE cur;
	DBMS_OUTPUT.PUT_LINE('******');
	DBMS_OUTPUT.PUT_LINE('new_user_id       new_user_name        Enabled        Role_ACTIONPLAN_APPROVER        Primary_Business_Unit');
	DBMS_OUTPUT.PUT_LINE('-----------------------------------------------------------------------------------------------------------');
	...
	open cur for select inf_obj_id, vu.username, decode(vu.enabled,  0, 'disabled', 1, 'enabled', 'unknown'), 
		...
		LOOP
			...
			IF (is_appr_role = 'false') THEN
				DBMS_OUTPUT.PUT_LINE('!!!!ERROR: New user ('|| new_us_id ||') should have ''ACTIONPLAN_APPROVER'' role!');
...
BEGIN
	...
	DBMS_OUTPUT.PUT_LINE('******');
	DBMS_OUTPUT.PUT_LINE('ACTION_PLAN_ID  approver_ID/username     AP_status       place_id      Primary_Business_Unit');
	DBMS_OUTPUT.PUT_LINE('--------------------------------------------------------------------------------------------');
	...
	OPEN select_cur;
		LOOP
			...
			if (select_rec.status != 'PENDINGAPPROVAL') then
				DBMS_OUTPUT.PUT_LINE('!!!!ERROR: Action plan ('|| select_rec.ACTION_PLAN_ID ||') should be in status ''Pending Approval''');
			...
			(select dimension_node_id from v_user_role_env where role_id = 211 and user_id = &new_user_id);
		...
		if(loop_count = wrong_user_count) then
			...
			DBMS_OUTPUT.PUT_LINE('Change ''current_approver_id'' either on the current approver user, or on ''0'' value, please!');
			raise ap_not_match_user;
		end if;
...
CREATE OR REPLACE procedure update_ap_approver(ap_id number, curr_us_id number, new_us_id number) as
	aplan_count int;
	update_approver_exception exception;
BEGIN
	INSERT INTO TMP_USER (ID, USER_ID) VALUES (1, 102);
	
	DBMS_OUTPUT.PUT_LINE(chr(10)||'*******v_a_ap_a_approvers_begin*******');
	IF curr_us_id = 0 THEN
		delete from v_a_ap_a_approvers where ACTION_PLAN_ID = ap_id;
		insert into v_a_ap_a_approvers (ACTION_PLAN_ID,USER_ID) values(ap_id, new_us_id);
		DBMS_OUTPUT.PUT_LINE('*******DELETE_INSERT v_a_ap_a_approvers for ap_id = ' || ap_id || '; new_us_id = '|| new_us_id || '*******');
	ELSE
		SELECT count(ACTION_PLAN_ID) into aplan_count FROM v_a_ap_a_approvers WHERE ACTION_PLAN_ID = ap_id AND user_id = curr_us_id;
		IF  (aplan_count = 0) THEN
			DBMS_OUTPUT.PUT_LINE('!!!!ERROR: The Action Plan ('|| ap_id ||') does not exist for approver ('|| curr_us_id ||')!');
			raise update_approver_exception;
		ELSE
			UPDATE v_a_ap_a_approvers SET user_id = new_us_id WHERE ACTION_PLAN_ID = ap_id AND user_id = curr_us_id;
			DBMS_OUTPUT.PUT_LINE('*******UPDATE v_a_ap_a_approvers for ap_id = ' || ap_id || '; curr_us_id=' || curr_us_id || '; new_us_id = '|| new_us_id || '*******');
		END IF;
	END IF;
	DBMS_OUTPUT.PUT_LINE('*******v_a_ap_a_approvers_end*******');
END;
/
...
begin
	...
	verify_AP(&a_p_id);
	update_ap_approver(&a_p_id, &curr_user_id, &new_user_id);
...
/
begin
	EXECUTE IMMEDIATE 'drop procedure update_ap_approver';
	Exception when others then null;
end;
/
begin
